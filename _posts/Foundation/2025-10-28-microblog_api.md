...
<!-- Interactive code editor: write code, run it in a sandbox, or export it -->
<section class="code-runner" aria-label="Interactive code editor" style="margin:1.5rem 0;">
<h2>Try Code</h2>
<p style="margin-bottom:0.5rem;color:#555;">Write JavaScript here. Click <strong>Run</strong> to execute in a sandboxed iframe, or <strong>Export</strong> to download the code as a file.</p>
<textarea id="codeArea" placeholder="// Write JavaScript here...\nconsole.log('hello world');" spellcheck="false" style="width:100%;height:260px;font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, 'Roboto Mono', 'Courier New', monospace; font-size:0.95rem; padding:12px; border:1px solid #d0d7de; border-radius:6px; background:#ffffff; color:#000000; overflow:auto; resize:vertical; caret-color:#000;"></textarea>
<div style="margin-top:0.6rem;">
    <button class="answer-btn" id="runBtn">Run</button>
    <button class="answer-btn" id="exportBtn">Export</button>
    <button class="answer-btn reset-btn" id="clearBtn">Clear</button>
</div>
<div id="codeOutput" style="margin-top:0.8rem;padding:12px;border-radius:6px;background:#ffffff;color:#000000;min-height:80px;white-space:pre-wrap;font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, monospace;overflow:auto;border:1px solid #d0d7de;">
<strong style="color:#111111;">Output will appear here.</strong>
</div>
...
;(function(){
    ...
    window.addEventListener('message', (ev)=>{
        try{
            const data = ev.data || {};
            if(data && data.type === 'console') appendOutput(data.text);
            if(data && data.type === 'error') appendOutput('Error: ' + data.text);
    ...
    runBtn.addEventListener('click', ()=>{
        ...
        // Build a small document that forwards console messages and errors to the parent.
        // We set the iframe's `srcdoc` instead of writing into its document to avoid
        // cross-origin access errors when the iframe is sandboxed.
        const safe = `<!doctype html><html><head><meta charset="utf-8"></head><body><script>\n(function(){\n  function send(t,p){ parent.postMessage({type:t,text:p}, '*'); }\n  ['log','info','warn','error'].forEach(function(m){ const orig = console[m]; console[m] = function(){ try{ send('console', Array.prototype.slice.call(arguments).join(' ')); }catch(e){}; if(orig) orig.apply(console, arguments); }; });\n  window.addEventListener('error', function(e){ try{ send('error', e && (e.message || e.toString()) ); }catch(e){} });\n  try{\n    var userCode = ${JSON.stringify(code)};\n    (new Function(userCode))();\n  }catch(e){ try{ send('error', e && e.toString()); }catch(err){} }\n})();<\/script></body></html>`;
        iframe.srcdoc = safe;
        // remove iframe after short delay to free resources
